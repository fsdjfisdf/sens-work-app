...
function fmtCapaValue(v){
  if(v==null || v==='') return '-';
  const n = Number(v);
  if(Number.isNaN(n)) return v;
  if(Math.abs(n) <= 1.5) return `${fmt1(toPct(n))}%`;
  return fmt1(n);
}
...
function barV(id, labels, datasets, opts={}){
  ...
  charts[id] = new Chart(el, {
    ...
    options:{
      ...
      plugins:{
        legend:{ display: opts.legendDisplay ?? true, position: opts.legendPosition || 'bottom' },
        datalabels:{
          display: opts.showDataLabels ?? true,
          anchor:'end',
          align:'end',
          clamp:true,
          clip:false,
        ...
        tooltip:{ callbacks:{ label:(c)=> opts.tooltipFormatter ? opts.tooltipFormatter(c) : `${c.dataset.label}: ${fmt1(c.raw)}` } }
      },
      scales:{
        x:{
          ticks:{ display: opts.xTickDisplay ?? true },
          grid:{ display: opts.xGridDisplay ?? true },
          border:{ display: opts.xBorderDisplay ?? true }
        },
        y:{
          beginAtZero:true,
          suggestedMax: opts.suggestedMax ?? undefined,
          ticks:{ display: opts.yTickDisplay ?? true },
          grid:{ display: opts.yGridDisplay ?? true },
          border:{ display: opts.yBorderDisplay ?? true }
        }
      }
    }
  });
...
async function load(){
  try{
    ...
    const fields = [
      ...
      ['SET UP CAPA', fmtCapaValue(p.setup_capa)], ['MAINT CAPA', fmtCapaValue(p.maint_capa)], ['MULTI CAPA', fmtCapaValue(p.multi_capa)],
      ['TOTAL CAPA', fmtCapaValue(p.capa)]
    ];
    ...
    // 11) Time rank (vertical, anonymous)
    const timeList = timeRank.slice(0, 30);
    ...
    barV('chart-time-rank', timeList.map(()=>''), [
      { label:'근무시간(h)', data:tVals, backgroundColor:tColors, borderRadius:8, maxBarThickness:26 }
    ], {
      legendDisplay:false,
      showDataLabels:false,
      xTickDisplay:false,
      xGridDisplay:false,
      xBorderDisplay:false,
      formatter:(v)=> `${fmt1(v)}h`,
      tooltipFormatter:(c)=> `근무시간: ${fmt1(c.raw)}h`
    });

    // 12) Task rank (vertical, anonymous)
    ...
    barV('chart-task-rank', taskList.map(()=>''), [
      { label:'작업건수', data:kVals, backgroundColor:kColors, borderRadius:8, maxBarThickness:26 }
    ], {
      legendDisplay:false,
      showDataLabels:false,
      xTickDisplay:false,
