...
/* Search */
async function doSearch(){const w=document.getElementById('table-wrapper');w.innerHTML='<div class="loading-state"><i class="fas fa-circle-notch fa-spin"></i> 조회 중...</div>';const f=getFilters(),p=new URLSearchParams();for(const[k,v]of Object.entries(f)){if(v)p.set(k,v);}
try{const{data}=await axios.get(`${API}/wl/events?${p}`);const{rows,total}=data;document.getElementById('result-summary').style.display='';document.getElementById('result-count').textContent=rows.length;if(!rows.length){w.innerHTML='<div class="empty-state"><i class="fas fa-inbox"></i><p>결과 없음</p></div>';return;}renderTable(w,rows);}catch(e){w.innerHTML=`<div class="empty-state"><p>조회 실패: ${escHtml(e.response?.data?.error||e.message)}</p></div>`;}}

function renderTable(c,rows){c.innerHTML=`<div class="table-scroll"><table class="wl-table"><thead><tr><th>Date</th><th>Work Code</th><th>Title</th><th>EQ Name</th><th>Group</th><th>Site</th><th>Work Type</th><th>EMS</th><th>Workers</th><th>RW</th></tr></thead><tbody>${rows.map(r=>`<tr data-id="${r.id}" class="clickable-row"><td>${fmtDate(r.task_date)}</td><td class="code-cell">${escHtml(r.work_code||'')}</td><td>${escHtml(r.task_name||'')}</td><td>${escHtml(r.equipment_name||'')}</td><td>${escHtml(r.group||'')}</td><td>${escHtml(r.site||'')}</td><td>${escHtml(r.work_type||'')}${r.work_type2?'/'+escHtml(r.work_type2):''}</td><td>${r.ems==1?'유상':'무상'}</td><td>${escHtml(r.workers_str||'')}</td><td>${r.is_rework?'<span class="rework-badge">RW</span>':''}</td></tr>`).join('')}</tbody></table></div>`;c.querySelectorAll('.clickable-row').forEach(tr=>tr.addEventListener('click',()=>openDetail(+tr.dataset.id)));}
...
/* Excel */
async function doExcel(){const f=getFilters(),p=new URLSearchParams();for(const[k,v]of Object.entries(f)){if(v)p.set(k,v);}
...
const hd=['Work Code','Title','Date','Country','Group','Site','Line','EQ Type','EQ Name','Warranty','EMS','Work Type','Work Sort','Setup Item','Status','Action','Cause','Result','SOP','TS Guide','Rework','Rework Seq','Worker','Role','Level','Start','End','None(분)','Move(분)','Duration(분)','Approval'];
const rows=data.map(r=>[r.work_code,r.task_name,r.task_date,r.country,r.group,r.site,r.line,r.equipment_type,r.equipment_name,r.warranty,r.ems_text,r.work_type,r.work_type2,r.setup_item,r.status,r.task_description,r.task_cause,r.task_result,r.SOP,r.tsguide,r.is_rework,r.rework_seq,r.engineer_name,r.role,r.eng_level,r.w_start_time?String(r.w_start_time).substring(0,5):'',r.w_end_time?String(r.w_end_time).substring(0,5):'',r.w_none_time,r.w_move_time,r.task_duration,r.approval_status]);
const wb=XLSX.utils.book_new(),ws=XLSX.utils.aoa_to_sheet([hd,...rows]);ws['!cols']=hd.map((h,i)=>{const mx=Math.max(h.length,...rows.map(r=>String(r[i]||'').length));return{wch:Math.min(mx+2,40)};});XLSX.utils.book_append_sheet(wb,ws,'Worklog');XLSX.writeFile(wb,`worklog_${new Date().toISOString().split('T')[0]}.xlsx`);toast('success','완료','다운로드됨');}catch(e){toast('error','실패',e.response?.data?.error||e.message);}}
