...
const API_BASE = "/api/pci/geneva-setup";
...
function exportPersonXlsx(){
  ...
  const name = (currentSummary?.worker || "worker") + "_GENEVA_SETUP_PCI.xlsx";
...
function exportMatrixXlsx(){
  ...
  const wb = XLSX.utils.book_new();
  const ws = XLSX.utils.aoa_to_sheet(aoa);
  XLSX.utils.book_append_sheet(wb, ws, "GENEVA SETUP PCI");
  XLSX.writeFile(wb, "GENEVA_SETUP_PCI_MATRIX.xlsx");
}
