...
const CATEGORIES = [
  ...
  { category: "EFEM Robot", items: [
    "SR8240 TEACHING","M124V TEACHING","M124C TEACHING",
    "EFEM ROBOT REP","EFEM ROBOT CONTROLLER REP"
  ]},
  { category: "TM Robot", items: [
    "SR8250 TEACHING","SR8232 TEACHING",
    "TM ROBOT REP","TM ROBOT CONTROLLER REP"
  ]},
  { category: "BM Module", items: [
    "PIN CYLINDER","PUSHER CYLINDER","DRT"
  ]},
  { category: "FFU (EFEM, TM)", items: [
    "FFU CONTROLLER","FAN","MOTOR DRIVER"
  ]},
  { category: "Microwave", items: [
    "MICROWAVE","APPLICATOR","APPLICATOR TUBE","MICROWAVE GENERATOR"
  ]},
  { category: "RF bias", items: [
    "MATCHER","RF GENERATOR"
  ]},
  { category: "Chuck", items: ["CHUCK"] },
  { category: "Process Kit", items: [
    "TOPLID PROCESS KIT","CHAMBER PROCESS KIT"
  ]},
  { category: "Leak", items: ["HELIUM DETECTOR"] },
  { category: "Pin", items: [
    "HOOK LIFT PIN","BELLOWS","PIN SENSOR","LM GUIDE",
    "HOOK LIFTER SERVO MOTOR","PIN MOTOR CONTROLLER"
  ]},
  { category: "EPD", items: ["SINGLE"] },
  { category: "Board", items: [
    "GAS BOX BOARD","POWER DISTRIBUTION BOARD","DC POWER SUPPLY",
    "BM SENSOR","PIO SENSOR","SAFETY MODULE","IO BOX","RACK BOARD","D-NET"
  ]},
  { category: "IGS Block", items: ["MFC","VALVE"] },
  { category: "Valve", items: [
    "SOLENOID","FAST VAC VALVE","SLOW VAC VALVE",
    "SLIT DOOR","APC VALVE","SHUTOFF VALVE"
  ]},
  { category: "ETC", items: [
    "BARATRON ASS'Y","PIRANI ASS'Y","VIEW PORT QUARTZ","FLOW SWITCH",
    "MONITOR","KEYBOARD","MOUSE","WATER LEAK DETECTOR","MANOMETER",
    "LIGHT CURTAIN","GAS SPRING"
  ]},
  { category: "CTR", items: [
    "CTC","PMC","EDA","EFEM CONTROLLER"
  ]},
...
function exportMatrixXlsx(){
  ...
  XLSX.utils.book_append_sheet(wb, ws, "ECOLITE PCI");
  XLSX.writeFile(wb, "ECOLITE_PCI_MATRIX.xlsx");
}
...
async function onFetchPerson(){
  ...
  try{
    const res = await axios.get(`/api/pci/ecolite/worker/${encodeURIComponent(name)}`);
...
function exportPersonXlsx(){
  ...
  const name = (currentSummary?.worker || "worker") + "_ECOLITE_PCI.xlsx";
...
async function openBreakdown(worker, item){
  try{
    const url = `/api/pci/ecolite/worker/${encodeURIComponent(worker)}/item/${encodeURIComponent(item)}`;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const bundle = new PciGroupBundle({
    storageKey: "PCI_ECOLITE_COMPARE",
    maxNames: 30,
