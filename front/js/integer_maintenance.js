document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (token) {
        try {
            const response = await axios.get('http://13.125.122.202:3001/integer-maintenance', {
                headers: {
    ...
    form.addEventListener('submit', async (event) => {
        ...
        const checklistFields = [
            ...
            'TOP_FEED_THROUGH', 'GAS_FEED_THROUGH', // ✅ 새 항목
            ...
            'PURGE_LINE_REGULATOR', // ✅ 새 항목
          
            'COOLING_CHUCK', 'HEATER_CHUCK',
            'GENERATOR',
            'D-NET_BOARD', 'SOURCE_BOX_BOARD', 'INTERFACE_BOARD', 'SENSOR_BOARD', 'PIO_SENSOR_BOARD', // ✅ 새 항목
        ...
        try {
            const response = await axios.post('http://13.125.122.202:3001/integer-maintenance', data, {
                headers: {
