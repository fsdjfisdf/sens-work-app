...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  let selectedReworkRefId = null;
  let selectedReworkRefLabel = '';
  let suspectReqSeq = 0;
  let suspectDebounce = null;
  let lastOpenedSuspectKey = '';
  let lastSuspectRows = [];
  let lastSuspectTotal = 0;
  ...
  function setReworkRefInfo(label = '') {
    ...
    box.textContent = `선택된 기준 이력: ${label}`;
  }
  ...
  function buildSuspectCardsHtml(rows) {
    return rows.map(row => {
      const refLabel = `${row.task_date || '-'} / ${row.site || '-'} / ${row.equipment_name || '-'} / ${row.task_name || '-'}`;
  ...
  function bindSuspectCardActions(root) {
    root?.querySelectorAll('.suspect-use-btn').forEach(btn => btn.addEventListener('click', () => {
  ...
  function clearReworkSuspects(resetRef = false) {
    const box = document.getElementById('rework-suspect-box');
    const msg = document.getElementById('rework-suspect-message');
    const openBtn = document.getElementById('rework-suspect-open');
    const modalList = document.getElementById('rework-suspect-list');
    const modalMsg = document.getElementById('rework-suspect-modal-message');
    if (box) box.classList.add('hidden');
    if (msg) msg.textContent = '2주 이내 동일 TITLE + 동일 CAUSE의 MAINT 이력이 있으면 여기 표시됩니다.';
    if (openBtn) openBtn.classList.add('hidden');
    if (modalList) modalList.innerHTML = '';
    if (modalMsg) modalMsg.textContent = '관련 작업이력 목록';
    lastSuspectRows = [];
    lastSuspectTotal = 0;
    lastOpenedSuspectKey = '';
    if (resetRef) {
      selectedReworkRefId = null;
      selectedReworkRefLabel = '';
      setReworkRefInfo('');
  ...
  function renderReworkSuspects(rows, total, options = {}) {
    const box = document.getElementById('rework-suspect-box');
    const msg = document.getElementById('rework-suspect-message');
    const openBtn = document.getElementById('rework-suspect-open');
    const modalList = document.getElementById('rework-suspect-list');
    const modalMsg = document.getElementById('rework-suspect-modal-message');
    if (!box || !msg || !openBtn || !modalList || !modalMsg) return;
    ...
    lastSuspectRows = rows.slice();
    lastSuspectTotal = total;

    box.classList.remove('hidden');
    msg.textContent = `2주 이내 동일 TITLE + 동일 CAUSE의 MAINT 이력이 ${total}건 있습니다. 관련 이력을 확인하고 REWORK 여부를 직접 판단하세요.`;
    openBtn.classList.remove('hidden');

    modalMsg.textContent = `2주 이내 동일 TITLE + 동일 CAUSE의 MAINT 이력 ${total}건`;
    modalList.innerHTML = buildSuspectCardsHtml(rows);
    bindSuspectCardActions(modalList);

    const modalKey = rows.map(r => r.id).join(',');
    if (options.autoOpen && modalKey && modalKey !== lastOpenedSuspectKey) {
      lastOpenedSuspectKey = modalKey;
      showOverlay('rework-suspect-modal');
    }
  }

  async function checkReworkCandidates(force = false) {
    const workType = getV('workType');
    const taskName = getV('task_name');
    const taskCause = getTaskCauseValue();
    const taskDate = getV('task_date');
    ...
    const seq = ++suspectReqSeq;
    box.classList.remove('hidden');
    msg.textContent = '2주 이내 동일 TITLE + 동일 CAUSE의 MAINT 이력을 확인 중입니다...';
    document.getElementById('rework-suspect-open')?.classList.add('hidden');

    try {
      const res = await axios.get(`${API}/wl/rework-candidates`, {
        params: { task_name: taskName, task_cause: taskCause, task_date: taskDate, days: 14, limit: 8 },
        headers: authH()
      });
      if (seq !== suspectReqSeq) return;
      const rows = Array.isArray(res.data?.rows) ? res.data.rows : [];
      const total = Number(res.data?.total) || rows.length;
      renderReworkSuspects(rows, total, { autoOpen: force || curStep === 4 });
    } catch (err) {
      if (seq !== suspectReqSeq) return;
      clearReworkSuspects(false);
      showToast('error', err.response?.data?.error || 'REWORK 의심 이력 조회 실패');
    }
  }
  ...
  /* ══ Overlay ══ */
  const overlay = document.getElementById('modal-overlay');
  const MODAL_IDS = ['preview-modal','result-modal','popup','picker-modal','rework-suspect-modal'];
  ...
  function showOverlay(modalId) {
    ...
    overlay.style.display = 'block';
    modal.style.display = modalId === 'picker-modal' ? 'block' : 'block';
    activeModal = modalId;
  }
  function hideAll() {
  ...
  document.getElementById('rework-suspect-open')?.addEventListener('click', () => {
    if (!lastSuspectRows.length) return;
    renderReworkSuspects(lastSuspectRows, lastSuspectTotal || lastSuspectRows.length, { autoOpen: false });
    showOverlay('rework-suspect-modal');
  });
  document.getElementById('rework-suspect-close')?.addEventListener('click', hideAll);
  document.getElementById('rework-suspect-close-footer')?.addEventListener('click', hideAll);
