...
async function loadWorkers(){
  const { data } = await axios.get("/api/specialist/ecolite/workers");
  const list = data?.workers || [];
  el.workerList.innerHTML = list.map(n=>`<option value="${esc(n)}"></option>`).join("");
}

async function fetchAll(worker){
  const { data } = await axios.get("/api/specialist/ecolite/edu?worker="+encodeURIComponent(worker));
...
async function saveOne(item){
  ...
  try{
    ...
    const { data } = await axios.patch("/api/specialist/ecolite/cell", {
