document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (token) {
        try {
            const response = await axios.get('http://13.125.122.202:3001/supraxp-maintenance', {
                headers: {
    ...
    form.addEventListener('submit', async (event) => {
        ...
        try {
            const response = await axios.post('http://13.125.122.202:3001/supraxp-maintenance', data, {
                headers: {
