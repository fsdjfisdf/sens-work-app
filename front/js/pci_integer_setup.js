...
const API_BASE = "/api/pci/integer-setup";
...
function exportPersonXlsx(){
  ...
  const name = (currentSummary?.worker || "worker") + "_INTEGER_SETUP_PCI.xlsx";
...
function exportMatrixXlsx(){
  ...
  const wb = XLSX.utils.book_new();
  const ws = XLSX.utils.aoa_to_sheet(aoa);
  XLSX.utils.book_append_sheet(wb, ws, "INTEGER SETUP PCI");
  XLSX.writeFile(wb, "INTEGER_SETUP_PCI_MATRIX.xlsx");
}
