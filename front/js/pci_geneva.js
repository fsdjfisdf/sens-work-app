...
async function loadWorkerList(){
  try{
    const res = await axios.get(`/api/pci/geneva/workers`);
...
async function buildMatrix(){
  ...
  try{
    const res = await axios.get(`/api/pci/geneva/matrix`);
...
function exportMatrixXlsx(){
  ...
  XLSX.utils.book_append_sheet(wb, ws, "GENEVA PCI");
  XLSX.writeFile(wb, "GENEVA_PCI_MATRIX.xlsx");
}
...
async function onFetchPerson(){
  ...
  try{
    const res = await axios.get(`/api/pci/geneva/worker/${encodeURIComponent(name)}`);
...
function exportPersonXlsx(){
  ...
  const name = (currentSummary?.worker || "worker") + "_GENEVA_PCI.xlsx";
...
async function openBreakdown(worker, item){
  try{
    const url = `/api/pci/geneva/worker/${encodeURIComponent(worker)}/item/${encodeURIComponent(item)}`;
