...
exports.fetchSetupLogsForSupraXP = async ({ startDate, endDate } = {}) => {
...
exports.fetchSelfRow = async (workerName) => {
  const [rows] = await pool.query(`SELECT * FROM SUPRA_XP_SETUP_SELF WHERE name = ? LIMIT 1`, [workerName]);
  return rows[0] || null;
};

/** 자가체크 전체: 매트릭스용 */
exports.fetchSelfAll = async () => {
  const [rows] = await pool.query(`SELECT * FROM SUPRA_XP_SETUP_SELF`);
  return rows;
};

/** 교육/가산 카운트 피벗: SUPRA_N_SETUP (카테고리 컬럼) → { "INSTALLATION PREPARATION": { "홍길동": n, ... } } */
exports.fetchAdditionalCountsPivot = async () => {
  const [rows] = await pool.query(`SELECT * FROM SUPRA_XP_SETUP`);
