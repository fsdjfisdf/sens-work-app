// src/pci/supran_setup/pciController.js
const { BASELINE, toDisplayCategory, workerAliases, CHECK_TITLES } = require("./pciConfig");
const { parseTaskMen, round1, clamp } = require("../precia/pciUtils"); // utils 재사용
const {
  fetchSetupLogsForSupraXP,
  fetchSelfRow,
  fetchSelfAll,
...
exports.getWorkerPci = async (req, res) => {
  try {
    ...
    const [logs, addPivot, selfRow] = await Promise.all([
      fetchSetupLogsForSupraXP({ startDate, endDate }),
      fetchAdditionalCountsPivot(),
      fetchSelfRow(worker),
    ]);
    ...
    for (const r of logs) {
      const item = toDisplayCategory(r.setup_item);
    ...
    console.error("[PCI SUPRA-XP SETUP] getWorkerPci error:", err);
...
exports.getAllSummary = async (req, res) => {
  try {
    ...
    console.error("[PCI SUPRA-XP SETUP] getAllSummary error:", err);
    return res.status(500).json({ message: "internal_error", detail: String(err?.message || err) });
  }
};
...
exports.getWorkerNames = async (_req, res) => {
  try {
    ...
    const set = new Set(selfRows.map(r => workerAliases(r.name)));
    for (const r of logs) for (const p of parseTaskMen(r.task_man)) set.add(workerAliases(p.name));
    for (const cat of Object.keys(addPivot)) for (const col of Object.keys(addPivot[cat])) set.add(workerAliases(col));
    ...
    console.error("[PCI SUPRA-XP SETUP] getWorkerNames error:", e);
...
exports.getMatrix = async (_req, res) => {
  try {
    ...
    console.error("[PCI SUPRA-XP SETUP] getMatrix error:", e);
    res.status(500).json({ message: "internal_error" });
  }
};
...
exports.getWorkerItemBreakdown = async (req, res) => {
  try {
    ...
    const worker = workerAliases(rawName);
    const itemDisp = toDisplayCategory(rawItem);
    if (!itemDisp || !BASELINE[itemDisp]) {
      return res.status(404).json({ message: `기준에 없는 항목: ${rawItem}` });
    }
    const baseline = BASELINE[itemDisp];

    const [logs, addPivot, selfRow] = await Promise.all([
      fetchSetupLogsForSupraXP({}), // 전체
      fetchAdditionalCountsPivot(),
      fetchSelfRow(worker),
    ]);
    ...
    for (const r of logs) {
      if (toDisplayCategory(r.setup_item) !== itemDisp) continue;
      const people = parseTaskMen(r.task_man);
      for (const p of people) {
        if (workerAliases(p.name) !== worker) continue;
        const role = p.weight >= 1 ? "main" : "support";
    ...
    console.error("[PCI SUPRA-XP SETUP] getWorkerItemBreakdown error:", err);
    return res.status(500).json({ message: "internal_error", detail: String(err?.message || err) });
  }
};
