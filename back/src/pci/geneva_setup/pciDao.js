...
exports.fetchSetupLogsForGeneva = async ({ startDate, endDate } = {}) => {
...
/** 자가체크 1행 (GENEVA_SETUP) */
exports.fetchSelfRow = async (workerName) => {
  const [rows] = await pool.query(`SELECT * FROM GENEVA_SETUP WHERE name = ? LIMIT 1`, [workerName]);
  return rows[0] || null;
};

/** 자가체크 전체 (매트릭스용) */
exports.fetchSelfAll = async () => {
  const [rows] = await pool.query(`SELECT * FROM GENEVA_SETUP`);
  return rows;
};

/** 교육/가산 카운트 피벗 (PRECIA_SETUP_COUNT) → { "INSTALLATION PREPARATION": { "홍길동": n, ... } } */
exports.fetchAdditionalCountsPivot = async () => {
  const [rows] = await pool.query(`SELECT * FROM GENEVA_SETUP_COUNT`);
