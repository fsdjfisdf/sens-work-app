...
exports.getWorkerPci = async (req, res) => {
  try {
    ...
    const [logs, addPivot, selfRow] = await Promise.all([
      fetchWorkLogsForPrecia({ startDate, endDate }),
      fetchAdditionalCountsPivot(),
    ...
    for (const item of Object.keys(BASELINE)) {
      ...
      // 자가체크 20%: 해당 컬럼 값이 1 이상이면 20, 아니면 0
      let selfPct = 0;
      if (selfRow) {
    ...
    console.error("[PCI PRECIA] getWorkerPci error:", err);
...
exports.getAllSummary = async (req, res) => {
  try {
    ...
    console.error("[PCI PRECIA] getAllSummary error:", err);
    return res.status(500).json({ message: "internal_error", detail: String(err?.message || err) });
  }
};
...
exports.getWorkerNames = async (_req, res) => {
  try {
    ...
    console.error("[PCI PRECIA] getWorkerNames error:", e);
  ...
  try {
    ...
    for (const r of logs) {
      ...
      for (const p of people) {
        ...
        const role = p.weight >= 1 ? "main" : "support";
        counts[w][item][role] += p.weight; // main=1.0 / support=0.1
      }
    }
    ...
    // 계산
    const data = {}; // data[item][worker] = {pci, work, self, total_count,...}
    const workerAvg = {}; // worker -> avg pci (모든 항목 0% 포함)
    ...
    console.error("[PCI PRECIA] getMatrix error:", e);
    res.status(500).json({ message: "internal_error" });
  }
};
...
exports.getWorkerItemBreakdown = async (req, res) => {
  try {
    ...
    console.error("[PCI PRECIA] getWorkerItemBreakdown error:", err);
    return res.status(500).json({ message: "internal_error", detail: String(err?.message || err) });
  }
};
