// src/pci/supraxp/pciDao.js
const { pool } = require("../../../config/database");
const { ALLOWED_EQUIP_TYPES, normalizeItem } = require("./pciConfig");

/** 기간 필터 포함 SUPRA XP 관련 로그 (equipment_type: SUPRA XP / SUPRA XQ) */
exports.fetchWorkLogsForSupraXP = async ({ startDate, endDate } = {}) => {
...
/** 자가체크 1행 (SUPRA_XP_MAINT_SELF) */
exports.fetchSelfCheckRow = async (workerName) => {
  const [rows] = await pool.query(
    `SELECT * FROM SUPRA_XP_MAINT_SELF WHERE name = ? LIMIT 1`,
    [workerName]
  );
  return rows[0] || null;
};

/** 교육/가산 카운트 피벗 (SUPRA_XP_MAINT_COUNT)
 *  예: { "LP ESCORT": { "정현우": 1, ... } }
 */
...
exports.fetchSelfCheckAll = async () => {
  const [rows] = await pool.query(`SELECT * FROM SUPRA_XP_MAINT_SELF`);
  return rows;
};
...
exports.fetchSelfCheckNames = async () => {
  const [rows] = await pool.query(
    `SELECT name FROM SUPRA_XP_MAINT_SELF WHERE name IS NOT NULL AND name <> ''`
  );
