...
exports.getWorkerPci = async (req, res) => {
  try {
    ...
    const [logs, addPivot, selfRow] = await Promise.all([
      fetchWorkLogsForSupraXP({ startDate, endDate }),
      fetchAdditionalCountsPivot(),
    ...
    console.error("[PCI SUPRA XP] getWorkerPci error:", err);
...
exports.getAllSummary = async (req, res) => {
  try {
    ...
    console.error("[PCI SUPRA XP] getAllSummary error:", err);
    return res.status(500).json({ message: "internal_error", detail: String(err?.message || err) });
  }
};
...
exports.getWorkerNames = async (req, res) => {
  try {
    ...
    console.error("[PCI SUPRA XP] getWorkerNames error:", e);
...
exports.getMatrix = async (req, res) => {
  try {
    ...
    // 계산
    const data = {};      // data[item][worker] = {...}
    const workerAvg = {}; // worker -> {s, n} (✅ 0% 포함)
    ...
    console.error("[PCI SUPRA XP] getMatrix error:", e);
    res.status(500).json({ message: "internal_error" });
  }
};
...
exports.getWorkerItemBreakdown = async (req, res) => {
  try {
    ...
    console.error("[PCI SUPRA XP] getWorkerItemBreakdown error:", err);
    return res.status(500).json({ message: "internal_error", detail: String(err?.message || err) });
  }
};
