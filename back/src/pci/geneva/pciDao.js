// src/pci/geneva/pciDao.js
const { pool } = require("../../../config/database");
const { ALLOWED_EQUIP_TYPES, normalizeItem } = require("./pciConfig");

/** 기간 필터 포함 GENEVA 관련 로그 */
exports.fetchWorkLogsForGeneva = async ({ startDate, endDate } = {}) => {
...
exports.fetchSelfCheckRow = async (workerName) => {
  const [rows] = await pool.query(
    `SELECT * FROM GENEVA_MAINT_SELF WHERE name = ? LIMIT 1`,
    [workerName]
  );
  return rows[0] || null;
};
...
exports.fetchSelfCheckAll = async () => {
  const [rows] = await pool.query(`SELECT * FROM GENEVA_MAINT_SELF`);
  return rows;
};
...
exports.fetchSelfCheckNames = async () => {
  const [rows] = await pool.query(
    `SELECT name FROM GENEVA_MAINT_SELF WHERE name IS NOT NULL AND name <> ''`
  );
