// src/pci/ecolite/pciDao.js
const { pool } = require("../../../config/database");
const { ALLOWED_EQUIP_TYPES, normalizeItem } = require("./pciConfig");

/** 기간 필터 포함 ECOLITE 관련 로그 */
exports.fetchWorkLogsForEcolite = async ({ startDate, endDate } = {}) => {
...
exports.fetchSelfCheckRow = async (workerName) => {
  const [rows] = await pool.query(
    `SELECT * FROM ECOLITE_MAINT_SELF WHERE name = ? LIMIT 1`,
    [workerName]
  );
  return rows[0] || null;
};
...
exports.fetchSelfCheckAll = async () => {
  const [rows] = await pool.query(`SELECT * FROM ECOLITE_MAINT_SELF`);
  return rows;
};

/** 자가체크 이름만 */
exports.fetchSelfCheckNames = async () => {
  const [rows] = await pool.query(`SELECT name FROM ECOLITE_MAINT_SELF WHERE name IS NOT NULL AND name <> ''`);
  return rows.map(r => r.name);
};

/** 교육/가산 카운트 피벗: { "LP ESCORT": { "홍길동": 1, ... } } */
exports.fetchAdditionalCountsPivot = async () => {
  const [rows] = await pool.query(`SELECT * FROM ECOLITE_MAINT_COUNT`);
