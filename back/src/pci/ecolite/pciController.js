...
exports.getWorkerPci = async (req, res) => {
  try {
    ...
    const [logs, addPivot, selfRow] = await Promise.all([
      fetchWorkLogsForEcolite({ startDate, endDate }),
      fetchAdditionalCountsPivot(),
    ...
    console.error("[PCI ECOLITE] getWorkerPci error:", err);
...
exports.getAllSummary = async (req, res) => {
  try {
    ...
    console.error("[PCI ECOLITE] getAllSummary error:", err);
    return res.status(500).json({ message: "internal_error", detail: String(err?.message || err) });
  }
};
...
exports.getWorkerNames = async (req, res) => {
  try {
    ...
    console.error("[PCI ECOLITE] getWorkerNames error:", e);
...
exports.getMatrix = async (req, res) => {
  try {
    ...
    // 교육/가산 합산
    const addCounts = {}; // addCounts[worker][item] = number
    for (const itemRaw of Object.keys(addPivot)) {
      const norm = normalizeItem(itemRaw);
      if (!BASELINE[norm]) continue;
    ...
    // 계산
    const data = {};      // data[item][worker] = {pci, work, self, ...}
    const workerAvg = {}; // worker -> {s, n} 모든 항목 포함(0% 포함)
    ...
    console.error("[PCI ECOLITE] getMatrix error:", e);
    res.status(500).json({ message: "internal_error" });
  }
};
...
exports.getWorkerItemBreakdown = async (req, res) => {
  try {
    ...
    console.error("[PCI ECOLITE] getWorkerItemBreakdown error:", err);
    return res.status(500).json({ message: "internal_error", detail: String(err?.message || err) });
  }
};
