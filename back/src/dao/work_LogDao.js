...
exports.addWorkLog = async (task_name, task_result, task_cause, task_man, task_description, task_date, start_time, end_time, none_time, move_time, group, site, SOP, tsguide, line, warranty, equipment_type, equipment_name, work_type, work_type2, setup_item, maint_item, transfer_item, task_maint, status) => {
  const connection = await pool.getConnection(async conn => conn);
  try {
    const query = `
    ...
    const values = [
      ...
      group, site, SOP, tsguide, line, warranty, equipment_type, equipment_name, work_type, work_type2, setup_item, maint_item, transfer_item, task_maint, status
    ];
...
exports.getSupraXPWorkLogs = async (equipment_type) => {
  ...
  try {
      ...
      const [rows] = await connection.query(query, [equipment_type]);  // 'SUPRA XP'만 필터링
      connection.release();
      return rows;
  } catch (err) {
      connection.release();
