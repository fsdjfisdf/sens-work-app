...
exports.insertChecklist = async (checklistData) => {
  ...
  try {
    ...
    INSERT INTO INTEGER_SETUP (
      ...
      DOCKING_REALIGNMENT, LEVELER_POSITION_UNDERSTANDING, MODULE_LEVELING, DOCKING_PIN_CHECK, HOOK_UP, TRAY_CHECK, 
      ...
      SAFETY_MODULE_UNDERSTANDING, EMO_CHECK, MODULE_CB_TURN_ON, SYCON_NUMBER_UNDERSTANDING, 
      ...
      NF3_LINE_LEAK_CHECK, H2_LINE_LEAK_CHECK, GAS_DILUTION_TEST, EFEM_ROBOT_PENDANT_CONTROL, EFEM_ROBOT_LEVELING, 
      EFEM_ROBOT_ARM_LEVELING, EFEM_ROBOT_PARAMETER_ADJUST, EFEM_TEACHING_DATA_SAVE, TM_ROBOT_PENDANT_CONTROL, 
      TM_ROBOT_LEVELING, TM_ROBOT_XYZ_VALUES, TM_ROBOT_PM_TEACHING, TM_ROBOT_AM_TEACHING, TM_TEACHING_DATA_SAVE, 
      WAFER_JIG_USE, LASER_JIG_USE, MARGIN_CHECK, SEMI_AUTO_TRANSFER, AGING_TEST, CERAMIC_PLATE_PIN_INSTALLATION, 
      PIN_HEIGHT_ADJUST, PIO_SENSOR_INSTALLATION, VIEW_PORT_COVER_INSTALLATION, LOAD_LOCK_LEVELING, 
      TM_ROBOT_PICK_INSTALLATION, TM_ROBOT_PICK_LEVELING, GAS_BOX_WINDOW_INSTALLATION, GAS_BOX_DAMPER_INSTALLATION, 
      LINE_MANUAL_LEAK_CHECK, MANUAL_LEAK_CHECK_HISTORY, HE_DETECTOR_USE, HE_BOTTLE_ORDER, 
      HE_DETECTOR_HOUSING_LEAK_CHECK, SLOT_VALVE_HE_LEAK_CHECK, VAC_CDA_SPEC_ADJUST, TEMP_PROFILE, 
      PUMP_VENT_TIME_ADJUST, EPD_PEAK_OFFSET_ADJUST, PM_BAFFLE_TEMP_AUTOTUNE, DOOR_VALVE_CONTROL, 
    ...
    const values = [
      ...
      checklistData.LEVELER_POSITION_UNDERSTANDING, checklistData.MODULE_LEVELING, checklistData.DOCKING_PIN_CHECK, 
      ...
      checklistData.SAFETY_MODULE_UNDERSTANDING, checklistData.EMO_CHECK, checklistData.MODULE_CB_TURN_ON, 
      checklistData.SYCON_NUMBER_UNDERSTANDING, checklistData.POWER_TURN_ON_ALARM_TROUBLESHOOTING, 
      checklistData.IP_ADDRESS_CHANGE, checklistData.UTILITY_TURN_ON_SEQUENCE, checklistData.VACUUM_TURN_ON, 
      checklistData.CDA_TURN_ON, checklistData.PCW_TURN_ON, checklistData.SOLENOID_VALVE_UNDERSTANDING, 
      checklistData.RELIEF_VALVE_UNDERSTANDING, checklistData.MANUAL_VALVE_UNDERSTANDING, checklistData.PUMP_TURN_ON, 
      checklistData.N2_PURGE_CHECK, checklistData.CHILLER_TURN_ON, checklistData.MANOMETER_LIMIT_ADJUST, 
      checklistData.GAS_TURN_ON, checklistData.GAS_TURN_ON_CHECK, checklistData.NF3_LINE_LEAK_CHECK, 
      checklistData.H2_LINE_LEAK_CHECK, checklistData.GAS_DILUTION_TEST, checklistData.EFEM_ROBOT_PENDANT_CONTROL, 
      checklistData.EFEM_ROBOT_LEVELING, checklistData.EFEM_ROBOT_ARM_LEVELING, checklistData.EFEM_ROBOT_PARAMETER_ADJUST, 
      checklistData.EFEM_TEACHING_DATA_SAVE, checklistData.TM_ROBOT_PENDANT_CONTROL, checklistData.TM_ROBOT_LEVELING, 
      checklistData.TM_ROBOT_XYZ_VALUES, checklistData.TM_ROBOT_PM_TEACHING, checklistData.TM_ROBOT_AM_TEACHING, 
      checklistData.TM_TEACHING_DATA_SAVE, checklistData.WAFER_JIG_USE, checklistData.LASER_JIG_USE, 
  ...
  try {
    const query = `
      UPDATE INTEGER_SETUP SET
        ...
        DOCKING_PIN_CHECK = ?, HOOK_UP = ?, TRAY_CHECK = ?, CABLE_SORTING = ?, GRATING_OPEN_CAUTION = ?, 
        LADDER_SAFETY_RULES = ?, CABLE_INSTALLATION = ?, CABLE_CONNECTION = ?, CABLE_TRAY_ARRANGEMENT = ?, 
        CABLE_CUTTING = ?, CABLE_RACK_CONNECTION = ?, PUMP_CABLE_TRAY = ?, PUMP_CABLE_ARRANGEMENT = ?, 
        CABLE_PM_PUMP_CONNECTION = ?, GPS_UPS_SPS_UNDERSTANDING = ?, POWER_TURN_ON_SEQUENCE = ?, RACK_CB_UNDERSTANDING = ?, 
        SAFETY_MODULE_UNDERSTANDING = ?, EMO_CHECK = ?, MODULE_CB_TURN_ON = ?, SYCON_NUMBER_UNDERSTANDING = ?, 
        POWER_TURN_ON_ALARM_TROUBLESHOOTING = ?, IP_ADDRESS_CHANGE = ?, UTILITY_TURN_ON_SEQUENCE = ?, 
        VACUUM_TURN_ON = ?, CDA_TURN_ON = ?, PCW_TURN_ON = ?, SOLENOID_VALVE_UNDERSTANDING = ?, 
        RELIEF_VALVE_UNDERSTANDING = ?, MANUAL_VALVE_UNDERSTANDING = ?, PUMP_TURN_ON = ?, N2_PURGE_CHECK = ?, 
        CHILLER_TURN_ON = ?, MANOMETER_LIMIT_ADJUST = ?, GAS_TURN_ON = ?, GAS_TURN_ON_CHECK = ?, 
        NF3_LINE_LEAK_CHECK = ?, H2_LINE_LEAK_CHECK = ?, GAS_DILUTION_TEST = ?, EFEM_ROBOT_PENDANT_CONTROL = ?, 
        EFEM_ROBOT_LEVELING = ?, EFEM_ROBOT_ARM_LEVELING = ?, EFEM_ROBOT_PARAMETER_ADJUST = ?, 
        EFEM_TEACHING_DATA_SAVE = ?, TM_ROBOT_PENDANT_CONTROL = ?, TM_ROBOT_LEVELING = ?, TM_ROBOT_XYZ_VALUES = ?, 
        TM_ROBOT_PM_TEACHING = ?, TM_ROBOT_AM_TEACHING = ?, TM_TEACHING_DATA_SAVE = ?, WAFER_JIG_USE = ?, 
        LASER_JIG_USE = ?, MARGIN_CHECK = ?, SEMI_AUTO_TRANSFER = ?, AGING_TEST = ?, 
        CERAMIC_PLATE_PIN_INSTALLATION = ?, PIN_HEIGHT_ADJUST = ?, PIO_SENSOR_INSTALLATION = ?, 
        VIEW_PORT_COVER_INSTALLATION = ?, LOAD_LOCK_LEVELING = ?, TM_ROBOT_PICK_INSTALLATION = ?, 
        TM_ROBOT_PICK_LEVELING = ?, GAS_BOX_WINDOW_INSTALLATION = ?, GAS_BOX_DAMPER_INSTALLATION = ?, 
        LINE_MANUAL_LEAK_CHECK = ?, MANUAL_LEAK_CHECK_HISTORY = ?, HE_DETECTOR_USE = ?, HE_BOTTLE_ORDER = ?, 
        HE_DETECTOR_HOUSING_LEAK_CHECK = ?, SLOT_VALVE_HE_LEAK_CHECK = ?, VAC_CDA_SPEC_ADJUST = ?, TEMP_PROFILE = ?, 
        PUMP_VENT_TIME_ADJUST = ?, EPD_PEAK_OFFSET_ADJUST = ?, PM_BAFFLE_TEMP_AUTOTUNE = ?, 
        DOOR_VALVE_CONTROL = ?, APC_AUTOLEARN = ?, PIN_SPEED_HEIGHT_ADJUST = ?, GAS_SUPPLY_PRESSURE_CHECK = ?, 
        GAS_EXHAUST_MONAMETER_CONTROL = ?, MFC_HUNTING_CHECK = ?, LP_FLOW_CONTROL = ?, AICP_POWER_CAL = ?, 
        PRODUCT_REPORT_COMPLETION = ?, TTTM_SHEET_COMPLETION = ?, LP_CERTIFICATION = ?, 
        FULL_PUMPING = ?, MID_OPERATION_CERTIFICATION_PREP = ?, LABEL_PLACEMENT = ?, 
        I_MARKING_PROCEDURE = ?, I_MARKING_LOCATION = ?, GAS_BOX_BOARD_LEVELING = ?, 
    ...
    const values = [
      ...
      checklistData.MODULE_LEVELING, checklistData.DOCKING_PIN_CHECK, checklistData.HOOK_UP, checklistData.TRAY_CHECK, 
      ...
      checklistData.POWER_TURN_ON_SEQUENCE, checklistData.RACK_CB_UNDERSTANDING, checklistData.SAFETY_MODULE_UNDERSTANDING, 
      checklistData.EMO_CHECK, checklistData.MODULE_CB_TURN_ON, checklistData.SYCON_NUMBER_UNDERSTANDING, 
      checklistData.POWER_TURN_ON_ALARM_TROUBLESHOOTING, checklistData.IP_ADDRESS_CHANGE, checklistData.UTILITY_TURN_ON_SEQUENCE, 
      checklistData.VACUUM_TURN_ON, checklistData.CDA_TURN_ON, checklistData.PCW_TURN_ON, checklistData.SOLENOID_VALVE_UNDERSTANDING, 
      checklistData.RELIEF_VALVE_UNDERSTANDING, checklistData.MANUAL_VALVE_UNDERSTANDING, checklistData.PUMP_TURN_ON, 
      checklistData.N2_PURGE_CHECK, checklistData.CHILLER_TURN_ON, checklistData.MANOMETER_LIMIT_ADJUST, 
      checklistData.GAS_TURN_ON, checklistData.GAS_TURN_ON_CHECK, checklistData.NF3_LINE_LEAK_CHECK, 
      checklistData.H2_LINE_LEAK_CHECK, checklistData.GAS_DILUTION_TEST, checklistData.EFEM_ROBOT_PENDANT_CONTROL, 
