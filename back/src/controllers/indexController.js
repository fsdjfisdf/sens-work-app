...
// 사용자 작업 이력 조회
exports.getUserWorkLogs = async function (req, res) {
  const userIdx = req.verifiedToken.userIdx;
  ...
  try {
    ...
    try {
      ...
      return res.status(200).json({
        isSuccess: true,
        code: 200,
        message: "사용자 작업 이력 조회 성공",
        result: { workLogs, totalHours },
      });
    } catch (err) {
      logger.error(`getUserWorkLogs Query error\n: ${JSON.stringify(err)}`);
      return res.status(500).json({
        isSuccess: false,
        code: 500,
        message: "서버 오류입니다.",
    ...
    logger.error(`getUserWorkLogs DB Connection error\n: ${JSON.stringify(err)}`);
    return res.status(500).json({
      isSuccess: false,
      code: 500,
      message: "서버 오류입니다.",
