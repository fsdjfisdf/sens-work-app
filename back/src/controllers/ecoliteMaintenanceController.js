const ecoliteMaintenanceDao = require('../dao/ecoliteMaintenanceDao');
...
exports.saveChecklist = async (req, res) => {
  ...
  try {
    ...
    // 사용자 정보 가져오기
    const user = await ecoliteMaintenanceDao.getUserById(userId);
    if (!user) {
    ...
    // 체크리스트 저장 또는 업데이트
    const existingEntry = await ecoliteMaintenanceDao.findByName(checklistData.name);
    if (existingEntry) {
      await ecoliteMaintenanceDao.updateChecklist(checklistData);
    } else {
      await ecoliteMaintenanceDao.insertChecklist(checklistData);
    }
...
exports.getChecklist = async (req, res) => {
  ...
  try {
    ...
    const checklist = await ecoliteMaintenanceDao.getChecklistByName(user.nickname);
    if (!checklist) {
...
exports.getAllChecklists = async (req, res) => {
  try {
    ...
    const allChecklists = await ecoliteMaintenanceDao.getAllChecklists();
