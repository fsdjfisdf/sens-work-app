const genevaMaintenanceDao = require('../dao/genevaMaintenanceDao');
...
exports.saveChecklist = async (req, res) => {
  ...
  try {
    ...
    // 사용자 정보 가져오기
    const user = await genevaMaintenanceDao.getUserById(userId);
    if (!user) {
    ...
    // 체크리스트 저장 또는 업데이트
    const existingEntry = await genevaMaintenanceDao.findByName(checklistData.name);
    if (existingEntry) {
      await genevaMaintenanceDao.updateChecklist(checklistData);
    } else {
      await genevaMaintenanceDao.insertChecklist(checklistData);
    }
...
exports.getChecklist = async (req, res) => {
  ...
  try {
    ...
    const checklist = await genevaMaintenanceDao.getChecklistByName(user.nickname);
    if (!checklist) {
...
exports.getAllChecklists = async (req, res) => {
  try {
    ...
    const allChecklists = await genevaMaintenanceDao.getAllChecklists();
