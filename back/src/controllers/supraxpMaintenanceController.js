const supraxpMaintenanceDao = require('../dao/supraxpMaintenanceDao');
...
exports.saveChecklist = async (req, res) => {
  ...
  try {
    ...
    // 사용자 정보 가져오기
    const user = await supraxpMaintenanceDao.getUserById(userId);
    if (!user) {
    ...
    // 체크리스트 저장 또는 업데이트
    const existingEntry = await supraxpMaintenanceDao.findByName(checklistData.name);
    if (existingEntry) {
      await supraxpMaintenanceDao.updateChecklist(checklistData);
    } else {
      await supraxpMaintenanceDao.insertChecklist(checklistData);
    }
...
exports.getChecklist = async (req, res) => {
  ...
  try {
    ...
    const checklist = await supraxpMaintenanceDao.getChecklistByName(user.nickname);
    if (!checklist) {
...
exports.getAllChecklists = async (req, res) => {
  try {
    ...
    const allChecklists = await supraxpMaintenanceDao.getAllChecklists();
