const hdwMaintenanceDao = require('../dao/hdwMaintenanceDao');
...
exports.saveChecklist = async (req, res) => {
  ...
  try {
    ...
    // 사용자 정보 가져오기
    const user = await hdwMaintenanceDao.getUserById(userId);
    if (!user) {
    ...
    // 체크리스트 저장 또는 업데이트
    const existingEntry = await hdwMaintenanceDao.findByName(checklistData.name);
    if (existingEntry) {
      await hdwMaintenanceDao.updateChecklist(checklistData);
    } else {
      await hdwMaintenanceDao.insertChecklist(checklistData);
    }
...
exports.getChecklist = async (req, res) => {
  ...
  try {
    ...
    const checklist = await hdwMaintenanceDao.getChecklistByName(user.nickname);
    if (!checklist) {
...
exports.getAllChecklists = async (req, res) => {
  try {
    ...
    const allChecklists = await hdwMaintenanceDao.getAllChecklists();
