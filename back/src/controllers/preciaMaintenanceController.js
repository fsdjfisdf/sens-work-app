const preciaMaintenanceDao = require('../dao/preciaMaintenanceDao');
...
exports.saveChecklist = async (req, res) => {
  ...
  try {
    ...
    // 사용자 정보 가져오기
    const user = await preciaMaintenanceDao.getUserById(userId);
    if (!user) {
    ...
    // 체크리스트 저장 또는 업데이트
    const existingEntry = await preciaMaintenanceDao.findByName(checklistData.name);
    if (existingEntry) {
      await preciaMaintenanceDao.updateChecklist(checklistData);
    } else {
      await preciaMaintenanceDao.insertChecklist(checklistData);
    }
...
exports.getChecklist = async (req, res) => {
  ...
  try {
    ...
    const checklist = await preciaMaintenanceDao.getChecklistByName(user.nickname);
    if (!checklist) {
...
exports.getAllChecklists = async (req, res) => {
  try {
    ...
    const allChecklists = await preciaMaintenanceDao.getAllChecklists();
