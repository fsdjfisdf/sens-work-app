...
function buildPrompt({ group, site, weekStart, kpis, topCause = [], topEq = [], incidents = [] }) {
  ...
  const facts = {
    ...
    incidents: incidents.slice(0, 5).map(x => ({
      task_date: x.task_date, equipment_name: x.equipment_name,
      task_name: x.task_name, task_cause: x.task_cause,
      status: x.status, hours: x.hours
    }))
  };
